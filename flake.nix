{
  ...
  outputs = inputs@{ self, nixpkgs, nixpkgs_22_05, home-manager, stylix, nixvim, nixos-hardware, flake-utils }:
  let
    allPackages = (
      system:
      ...
      in
        import nix/default.nix { inherit pkgs; old_pkgs = {"22_05" = pkgs_22_05; }; }
    );
  in
  ...
  //
  {
    nixosConfigurations = {
      ...
      "nixos-dell-xps" = nixpkgs.lib.nixosSystem {
        ...
        specialArgs = { inherit inputs stylix nixvim; packages = allPackages "x86_64-linux"; };
      };
    };
  };
}
